...
const apiClient = axios.create({
    baseURL: import.meta.env.VITE_API_AUTH_URL, // Đặt base URL cho API của bạn
    headers: {
        'Content-Type': 'application/json',
    },
    timeout: 30000, // Thời gian timeout cho request (30 giây)
});
...
apiClient.interceptors.response.use(
    (response) => {
        ...
        return res; // Trả về data trong response
    },
    (error) => {
        var response = {
            header: {
        ...
        if (error.status == 400) {
            response.header.message = error.response.status + ": " + error.response.data.message;
        }
