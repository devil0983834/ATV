...
const chartOptionsLine = ref({
    ...
    plotOptions: {
        ...
        column: {
            borderRadius: 1,
            stacking: 'normal',
            //  pointWidth: 40, // Độ rộng của mỗi cột (pixels)
            dataLabels: {
                enabled: true,
                align: 'center', // Căn giữa
                ...
                formatter: function () {
                    if (_.toString(this.point.percentage) != "") {
                        if (this.series.name == 'OUTPUT') {
                           return Intl.NumberFormat('en-US').format(this.y);
                        }
        ...
        chartOptionsLine.value = {
            ...
            yAxis: [
                {   // Primary yAxis
                    ...
                    min: 0, // Bắt đầu từ 0
                    opposite: true,
                    gridLineWidth: 0, // Ẩn grid line
                    tickPositioner: function () {
                        let ticks = [];
                        let step = Math.ceil(this.max / 5) * 2;
                        for (let i = 0; i <= 5; i += 1) { // Điều chỉnh khoảng cách tick tùy theo dữ liệu
                            ticks.push(i * step);
                        }
                        return ticks;
                    },
                    ...
                    labels: {
                        ...
                        formatter: function () {
                            return this.value.toLocaleString('vi-VN'); // Định dạng kiểu Việt Nam: 1.234.567,89
                        },
                    },
