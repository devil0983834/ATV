...
const GetdataApi = async () => {
    ...
    const response = await apiService.post('/api/UeMtba/GetDataMTBA', gFunc.CreateReqData(paramReq));
    var res = gFunc.CheckResData(response);
    if (res.isCheck) {
        InitData();
...
const Convertdata = (resData) => {
    ...
    const countItem = chartDaUe.data.length > chartWbUe.data.length ? chartDaUe.data.length : chartWbUe.data.length;
    const chartTarget = {
        name: "TARGET",
        color: gVariable.borderColor[0],
        data: new Array(countItem).fill(selectedTarget.value.code),
    }

    // set chart data
    dataDaUe.value.chartData = [chartDaUe, chartTarget];
    dataDaMtba.value.chartData = [chartDaMtba, chartTarget];
    dataWbUe.value.chartData = [chartWbUe, chartTarget];
    dataWbMtba.value.chartData = [chartWbMtba, chartTarget];
...
const AddNewTarget = (event) => {
    ...
    const exists = lstTarget.value.some(
        (target) => target.name.toString().toLowerCase() === currentTarget.toLowerCase()
    );
    ...
    setTimeout(() => {
        if (!event.query.trim().length) {
            ...
            filteredTarget.value = lstTarget.value.filter((target) => {
                ...
                return target.name.toString().toLowerCase().includes(event.query.toLowerCase());
            });
        }
    }, 200);
}
...
watch(currentInputTarget, async (newValue, oldValue) => {
    showAddNewTarget.value = newValue.trim().length > 0 && !lstTarget.value.some(
        (target) => target.name.toString().toLowerCase() === newValue.trim().toLowerCase()
    );
}, { immediate: true });

watch(selectedTarget, async (newValue, oldValue) => {
    if (dataDaUe.value.chartData.length > 0) {
        const countItem = dataDaUe.value.chartData[0].data.length > dataWbUe.value.chartData[0].data.length ? dataDaUe.value.chartData[0].data.length : dataWbUe.value.chartData[0].data.length;
        const chartTarget = {
            name: "TARGET",
            color: gVariable.borderColor[0],
            data: new Array(countItem).fill(newValue.code),
            // chartDaUe.data.length > chartWbUe.data.length? chartDaUe.data.length: chartWbUe.data.length
        }
        // set chart data
...
<template>
    <div class="col-span-12 lg:col-span-12 xl:col-span-12 my-2">
        <div class="flex items-center mt-3 gap-3">
            ...
            <FloatLabel class="w-[150px]" variant="on">
                <AutoComplete dropdown v-model="selectedTarget" optionLabel="name" :suggestions="filteredTarget"
                ...
                <label for="target">Current target</label>
            </FloatLabel>
            <Button label="Save Target" severity="info" :loading="isLoading" v-on:click="SaveTarget()"
                icon="pi pi-save px-1" size="small"></Button>
